...
eval' term =
  case term of
    TmDense n m -> return $ TmDense n m
    TmCon t1 t2 -> do
        t1' <- eval' t1
        t2' <- eval' t2
        return $ TmCon t1' t2'
    ...
    TmLet name t1 t2 -> do
        ...
        evalState (return $ eval' t2) innerState

-- | Tests whether a given term is a value
isVal :: Term -> Bool
isVal (TmDense _ _) = True
isVal _ = False

typeof :: Term -> EvalState Type
typeof term = 
  case term of
    TmDense n m -> return $ TyNetwork n m
    TmCon t1 t2 -> do
      left <- sizeLeft t1
      right <- sizeRight t2
...
sizeLeft term = 
  case term of 
    TmDense _ m -> return m 
    TmCon t1 t2 -> sizeLeft t1 
